<x-guest-layout>
    ...
    @if(!Auth::user())
        <div class="max-w-sm p-8 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Você precisa estar logado para responder</h5>
    ...
    @else
        ...
        <p class="text-lg py-3">{{$question->body}}</p>

        <form class="lg:w-[50%]" method="POST" action="{{route('answer.store')}}">
            @csrf
